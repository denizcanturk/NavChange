...
PLOT_DOWNSAMPLE = 100     # Performans için örnekleme oranı

class CockpitApp:
    def __init__(self, root, datafile):
        self.root = root
        self.root.title("Flight Data Recorder - Final Clean Edition")
    ...
    def load_data(self, filename):
        try:
            ...
            print(f"Hazır! {self.total_frames} kayıt.")
    ...
    def create_layout(self):
        ...
        # A) Göstergeler (En Üst)
        self.gauge_frame = tk.Frame(self.sidebar_pane, bg="#202020")
        self.sidebar_pane.add(self.gauge_frame, minsize=200) # Esnek yükseklik
        ...
        # B) Rate Grafiği (Altta - Artık daha geniş yer kaplayacak)
    ...
    def create_rate_plot(self):
        ...
        self.axRate.set_title("Angular Rates (Deg/s)", color='gray', fontsize=9)
    ...
    def update_ui(self):
        ...
        self.lbl_roll_rate.config(text=f"Cur: {row['RollRate']:.2f}°/s | Max: {row['RollRate_Max']:.2f}")
        self.lbl_pitch_rate.config(text=f"Cur: {row['PitchRate']:.2f}°/s | Max: {row['PitchRate_Max']:.2f}")
        self.lbl_yaw_rate.config(text=f"Cur: {row['YawRate']:.2f}°/s | Max: {row['YawRate_Max']:.2f}")
    ...
    def draw_attitude(self, roll, pitch):
        c = self.canvas_attitude; c.delete("all")
        cx, cy, r = 80, 80, 70
        c.create_oval(cx-r, cy-r, cx+r, cy+r, fill="#00BFFF", outline="")
        pitch_scale = 1.2; offset = pitch * pitch_scale
        w, h = 300, 300
        pts = [(-w, offset), (w, offset), (w, h), (-w, h)]
        rad = math.radians(-roll); cos_a, sin_a = math.cos(rad), math.sin(rad)
        r_pts = []
        for x, y in pts: r_pts.extend([x*cos_a - y*sin_a + cx, x*sin_a + y*cos_a + cy])
        ...
        c.create_line(cx-20, cy, cx-5, cy, width=2, fill="yellow")
        c.create_line(cx+5, cy, cx+20, cy, width=2, fill="yellow")
        c.create_text(cx, cy-50, text=f"R:{roll:.0f}", fill="white", font=("Arial", 8))
